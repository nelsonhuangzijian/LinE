...
class LineNegation(nn.Module):
    def __init__(self):
        super(LineNegation, self).__init__()

    def forward(self, embeddings):
        embeddings = 1. / (embeddings+1e-10)

        return embeddings

class LineUnion(nn.Module):
    def __init__(self):
        super(LineUnion, self).__init__()
...
class LineIntersection(nn.Module):
    def __init__(self, dim, att):
        super(LineIntersection, self).__init__()
        self.dim = dim
        self.layer1 = nn.Linear(self.dim, self.dim)
        self.layer2 = nn.Linear(self.dim, self.dim)
        self.att = att
    ...
    def forward(self, embeddings):
        if self.att == 'T':
            ...
            embeddings = torch.min(attention * embeddings, dim=0).values
        else:
            embeddings = torch.min(embeddings, dim=0).values

        return embeddings

class LineProjection(nn.Module):
    def __init__(self, entity_dim, relation_dim, hidden_dim, projection_regularizer, num_layers):
        super(LineProjection, self).__init__()
        self.entity_dim = entity_dim
        self.relation_dim = relation_dim
        self.hidden_dim = hidden_dim
        self.num_layers = num_layers
...
class KGReasoning(nn.Module):
    def __init__(self, nentity, nrelation, hidden_dim, gamma, 
                 geo, data_path, number_of_points, query_loss, attention, test_batch_size=1,
                 box_mode=None, use_cuda=False,
                 query_name_dict=None, beta_mode=None, line_mode=None):
        ...
        if self.geo == 'box':
            ...
            self.entity_embedding = nn.Parameter(torch.zeros(nentity, self.entity_dim * 2))  # alpha and beta
            self.entity_regularizer = Regularizer(1, 0.05, 1e9)  # make sure the parameters of beta embeddings are positive
            self.line_regularizer = Regularizer(0.25, 0.01, 1e9)
        nn.init.uniform_(
        ...
        if self.geo == 'box':
            ...
            hidden_dim, num_layers = line_mode
            self.negation_net = LineNegation()
            self.center_net = LineIntersection(self.entity_dim * self.number_of_points, self.attention)
            self.projection_net = LineProjection(self.entity_dim * self.number_of_points,
                                                 self.relation_dim,
                                                 hidden_dim,
                                                 self.line_regularizer,
                                                 num_layers)
            self.union_net = LineUnion()

            layers = nn.Sequential(
                nn.Linear(2, 16),
                nn.ReLU(),
                nn.Linear(16, self.number_of_points)
            )
            alpha_embedding, beta_embedding = torch.chunk(self.entity_regularizer(self.entity_embedding), 2, dim=-1)
            x = torch.stack((alpha_embedding, beta_embedding), dim=-1)
            self.line_ent_embedding = nn.Parameter(self.line_regularizer(layers(x)).reshape(self.nentity, self.entity_dim * self.number_of_points))
    ...
    def forward(self, positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict):
        if self.geo == 'box':
            ...
            return self.forward_line(positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict)
    ...
    def embed_query_line(self, queries, query_structure, idx):
        all_relation_flag = True
        ...
        if all_relation_flag:
            if query_structure[0] == 'e':
                # transform beta to line
                embedding = torch.index_select(self.line_ent_embedding, dim=0, index=queries[:, idx])
                idx += 1
            else:
                embedding, idx = self.embed_query_line(queries, query_structure[0], idx)

            for i in range(len(query_structure[-1])):
                if query_structure[-1][i] == 'n':
                    # Negation
                    embedding = self.negation_net(embedding)
                elif query_structure[-1][i] == 'u':
                    print("xxx")
                    # Union
                    embedding_list = []
                    idx -= 2
                    ...
                    for j in range(len(query_structure)):
                        embedding, idx = self.embed_query_line(queries, query_structure[j], idx)
                        embedding_list.append(embedding)
                    embedding = self.union_net(torch.stack(embedding_list))
                else:
                    # Projection
                    r_embedding = torch.index_select(self.relation_embedding, dim=0, index=queries[:, idx])
                    embedding = self.projection_net(embedding, r_embedding)
            ...
            embedding_list = []
            for i in range(len(query_structure)):
                embedding, idx = self.embed_query_line(queries, query_structure[i], idx)
                embedding_list.append(embedding)
            embedding = self.center_net(torch.stack(embedding_list))

        return embedding, idx

    def cal_logit_line(self, entity_embedding, query_line):
        loss = nn.MSELoss(reduction='none')
        if len(entity_embedding.shape) > 3:
            entity_embedding = entity_embedding.squeeze(0)
            query_line = query_line.squeeze(0)
            if len(query_line.shape) > 3:
                entity_embedding_list = torch.chunk(entity_embedding, entity_embedding.shape[2], dim=2)
                query_line_list = torch.chunk(query_line, query_line.shape[1], dim=1)
                logit_list = []
                for inp in query_line_list:
                    output_list = []
                    for target in entity_embedding_list:
                        mse = loss(inp, target)
                        output = (self.gamma - torch.norm(mse, p=1, dim=-1))
                        output_list.append(output)
                    new_output = output_list[0]
                    for i in range(1, len(output_list)):
                        b = output_list[i]
                        new_output = torch.cat((new_output, b), dim=-1)
                    logit_list.append(new_output)
                c = logit_list[0]
                if len(logit_list) > 1:
                    for i in range(1, len(logit_list)):
                        c = torch.cat((c, logit_list[i]), 1)
                logit = c
            else:
                entity_embedding_list = torch.chunk(entity_embedding, entity_embedding.shape[1], dim=1)
                query_line_list = torch.chunk(query_line, query_line.shape[0], dim=0)
                logit_list = []
                for target in entity_embedding_list:
                    output_list = []
                    for inp in query_line_list:
                    ...
                    for i in range(1, len(output_list)):
                        b = output_list[i]
                        new_output = torch.cat((new_output, b), dim=0)
                    logit_list.append(new_output)
                c = logit_list[0]
                if len(logit_list) > 1:
                    for i in range(1, len(logit_list)):
    ...
    def cal_query_logit(self, inp, target):
        loss = nn.MSELoss()
        logit = loss(inp, target)

        return logit

    def cal_order_logit(self, inp, target):
        loss = nn.HingeEmbeddingLoss()
        logit = loss(inp, target)

        return logit

    def forward_line(self, positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict):
        all_idxs, all_line_embeddings = [], []
        all_union_idxs, all_union_line_embeddings = [], []
        hierarchical_proj_embeddings, hierarchical_proj_idxs = [], []
        reverse_hierarchical_proj_embeddings, reverse_hierarchical_proj_idxs = [], []
        non_hierarchical_proj_embeddings, non_hierarchical_proj_idxs = [], []
        all_inter_embeddings, all_inter_idxs = [], []
        if 'FB15k' in self.data_path:
            if '237' in self.data_path:
                hierarchical_relations = [30, 156, 274, 414]
                reverse_hierarchical_relations = [31, 157, 275, 415]
        elif 'NELL' in self.data_path:
            hierarchical_relations = [44, 132, 266]
            reverse_hierarchical_relations = [45, 133, 267]
        elif 'WN18RR' in self.data_path:
            hierarchical_relations = [0, 4, 8, 12, 14, 16]
            reverse_hierarchical_relations = [1, 5, 9, 13, 15, 17]
        for query_structure in batch_queries_dict:
            if 'u' in self.query_name_dict[query_structure] and 'DNF' in self.query_name_dict[query_structure]:
                embedding, _ = \
                    self.embed_query_line(self.transform_union_query(batch_queries_dict[query_structure],query_structure),
                                          self.transform_union_structure(query_structure),
                                          0)
                all_union_idxs.extend(batch_idxs_dict[query_structure])
                all_union_line_embeddings.append(embedding)
            else:
                embedding, _ = self.embed_query_line(batch_queries_dict[query_structure],
                                                     query_structure,
                                                     0)
                all_idxs.extend(batch_idxs_dict[query_structure])
                all_line_embeddings.append(embedding)

                if self.query_loss == 'T':
                    if (query_structure == ('e', ('r',))) or (query_structure == ('e', ('r', 'r'))) \
                            or (query_structure == ('e', ('r', 'r', 'r'))):
                        for query in batch_queries_dict[query_structure]:
                            if len(query) == 2:
                                if query[1] in hierarchical_relations:
                                    hierarchical_proj_idxs.extend(batch_idxs_dict[query_structure])
                                    hierarchical_proj_embeddings.append(embedding)
                                elif query[1] in reverse_hierarchical_relations:
                                    reverse_hierarchical_proj_idxs.extend(batch_idxs_dict[query_structure])
                                    reverse_hierarchical_proj_embeddings.append(embedding)
                                else:
                                    non_hierarchical_proj_idxs.extend(batch_idxs_dict[query_structure])
                                    non_hierarchical_proj_embeddings.append(embedding)
                            elif len(query) == 3:
                                if (query[1] or query[2]) in hierarchical_relations:
                                    hierarchical_proj_idxs.extend(batch_idxs_dict[query_structure])
                                    hierarchical_proj_embeddings.append(embedding)
                                elif (query[1] or query[2]) in reverse_hierarchical_relations:
                                    reverse_hierarchical_proj_idxs.extend(batch_idxs_dict[query_structure])
                                    reverse_hierarchical_proj_embeddings.append(embedding)
                                else:
                                    non_hierarchical_proj_idxs.extend(batch_idxs_dict[query_structure])
                                ...
                                if (query[1] or query[2] or query[3]) in hierarchical_relations:
                                    hierarchical_proj_idxs.extend(batch_idxs_dict[query_structure])
                                    hierarchical_proj_embeddings.append(embedding)
                                elif (query[1] or query[2] or query[3]) in reverse_hierarchical_relations:
                                    reverse_hierarchical_proj_idxs.extend(batch_idxs_dict[query_structure])
                                    reverse_hierarchical_proj_embeddings.append(embedding)
                                else:
                                    non_hierarchical_proj_idxs.extend(batch_idxs_dict[query_structure])
                                    non_hierarchical_proj_embeddings.append(embedding)
                    elif (query_structure == (('e', ('r',)), ('e', ('r',)))) or \
                            (query_structure == (('e', ('r',)), ('e', ('r',)), ('e', ('r',)))):
                        all_inter_idxs.extend(batch_idxs_dict[query_structure])
                        all_inter_embeddings.append(embedding)

        if len(all_line_embeddings) > 0:
            all_line_embeddings = torch.cat(all_line_embeddings, dim=0).unsqueeze(1)
        if len(all_union_line_embeddings) > 0:
            all_union_line_embeddings = torch.cat(all_union_line_embeddings, dim=0).unsqueeze(1)
            all_union_line_embeddings = all_union_line_embeddings.view(all_union_line_embeddings.shape[0]//2, 2, 1, -1)
        ...
        if type(positive_sample) != type(None):
            if len(all_line_embeddings) > 0:
                ...
                positive_embedding = torch.index_select(self.line_ent_embedding, dim=0, index=positive_sample_regular).unsqueeze(1)
                positive_logit = self.cal_logit_line(positive_embedding, all_line_embeddings)
            else:
                positive_logit = torch.Tensor([]).to(self.entity_embedding.device)

            if len(all_union_line_embeddings) > 0:
                positive_sample_union = positive_sample[all_union_idxs]  # positive samples for union queries in this batch
                positive_embedding = torch.index_select(self.line_ent_embedding, dim=0, index=positive_sample_union).unsqueeze(1).unsqueeze(1)
                positive_union_logit = self.cal_logit_line(positive_embedding, all_union_line_embeddings)
        ...
        if type(negative_sample) != type(None):
            if len(all_line_embeddings) > 0:
                ...
                negative_embedding = torch.index_select(self.line_ent_embedding, dim=0, index=negative_sample_regular.view(-1)).view(
                    batch_size, negative_size, -1)
                negative_logit = self.cal_logit_line(negative_embedding, all_line_embeddings)
            else:
            ...
            if len(all_union_line_embeddings) > 0:
                ...
                negative_embedding = torch.index_select(self.line_ent_embedding, dim=0, index=negative_sample_union.view(-1)).view(
                    batch_size, 1, negative_size, -1)
                negative_union_logit = self.cal_logit_line(negative_embedding, all_union_line_embeddings)
        ...
        # query loss
        if self.query_loss == 'T':
            if len(hierarchical_proj_embeddings) > 0:
                hierarchical_proj_embeddings = torch.cat(hierarchical_proj_embeddings, dim=0).unsqueeze(1)
            if len(reverse_hierarchical_proj_embeddings) > 0:
                reverse_hierarchical_proj_embeddings = torch.cat(reverse_hierarchical_proj_embeddings, dim=0).unsqueeze(1)
            if len(non_hierarchical_proj_embeddings) > 0:
                non_hierarchical_proj_embeddings = torch.cat(non_hierarchical_proj_embeddings, dim=0).unsqueeze(1)
            if len(all_inter_embeddings) > 0:
                all_inter_embeddings = torch.cat(all_inter_embeddings, dim=0).unsqueeze(1)

            if type(positive_sample) != type(None):
                # projection loss
                if len(hierarchical_proj_embeddings) > 0:
                    hierarchical_positive_proj_sample_regular = positive_sample[hierarchical_proj_idxs]
                    hierarchical_positive_proj_embedding = torch.index_select(self.line_ent_embedding, dim=0,
                                                                              index=hierarchical_positive_proj_sample_regular).unsqueeze(1)
                    hierarchical_proj_mse_loss = self.cal_query_logit(hierarchical_proj_embeddings,
                                                                      hierarchical_positive_proj_embedding)
                    hierarchical_proj_order_loss = self.cal_order_logit(hierarchical_proj_embeddings,
                                                                        hierarchical_positive_proj_embedding)
                    hierarchical_proj_loss = hierarchical_proj_mse_loss + hierarchical_proj_order_loss
                else:
                    hierarchical_proj_loss = torch.zeros(1).to(self.entity_embedding.device)
                if len(reverse_hierarchical_proj_embeddings) > 0:
                    reverse_hierarchical_positive_proj_sample_regular = positive_sample[reverse_hierarchical_proj_idxs]
                    reverse_hierarchical_positive_proj_embedding = torch.index_select(self.line_ent_embedding, dim=0,
                                                                           index=reverse_hierarchical_positive_proj_sample_regular).unsqueeze(1)
                    reverse_hierarchical_proj_mse_loss = self.cal_query_logit(reverse_hierarchical_positive_proj_embedding,
                                                                              reverse_hierarchical_proj_embeddings)
                    reverse_hierarchical_proj_order_loss = self.cal_order_logit(reverse_hierarchical_positive_proj_embedding,
                                                                                reverse_hierarchical_proj_embeddings)
                    reverse_hierarchical_proj_loss = reverse_hierarchical_proj_mse_loss + reverse_hierarchical_proj_order_loss
                else:
                    reverse_hierarchical_proj_loss = torch.zeros(1).to(self.entity_embedding.device)
                if len(non_hierarchical_proj_embeddings) > 0:
                    positive_proj_sample_regular_nh = positive_sample[non_hierarchical_proj_idxs]
                    positive_proj_embedding_nh = torch.index_select(self.line_ent_embedding, dim=0,
                                                                    index=positive_proj_sample_regular_nh).unsqueeze(1)
                    non_hierarchical_proj_loss = self.cal_query_logit(positive_proj_embedding_nh,
                                                                      non_hierarchical_proj_embeddings)
                else:
                    non_hierarchical_proj_loss = torch.zeros(1).to(self.entity_embedding.device)
                proj_loss = hierarchical_proj_loss + reverse_hierarchical_proj_loss + non_hierarchical_proj_loss
                # intersection loss
                if len(all_inter_embeddings) > 0:
                    positive_inter_sample_regular = positive_sample[all_inter_idxs]
                    positive_inter_embedding = torch.index_select(self.line_ent_embedding, dim=0,
                                                                  index=positive_inter_sample_regular).unsqueeze(1)
                    inter_loss = self.cal_query_logit(positive_inter_embedding, all_inter_embeddings)
                else:
                    inter_loss = torch.zeros(1).to(self.entity_embedding.device)
                # query_logit = torch.Tensor([proj_loss.item()]).to(self.entity_embedding.device)
                query_logit = torch.Tensor([proj_loss.item(), inter_loss.item()]).to(self.entity_embedding.device)
            else:
                query_logit = None
        else:
            query_logit = None

        return positive_logit, negative_logit, query_logit, subsampling_weight, all_idxs + all_union_idxs
    ...
    def train_step(model, optimizer, train_iterator, args, step):
        ...
        if args.geo == 'line':
            positive_logit, negative_logit, query_logit, subsampling_weight, _ = model(positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict)
        else:
        ...
        if args.query_loss == 'T':
            proj_loss, inter_loss = torch.chunk(query_logit, 2, dim=0)
            query_loss = proj_loss + inter_loss
            loss = (positive_sample_loss + negative_sample_loss) / 2 + query_loss / 2
        else:
            query_loss = torch.Tensor([0.])
    ...
    def test_step(model, easy_answers, hard_answers, args, test_dataloader, query_name_dict, save_result=False, save_str="", save_empty=False):
        ...
        with torch.no_grad():
            for negative_sample, queries, queries_unflatten, query_structures in tqdm(test_dataloader, disable=not args.print_on_screen):
                ...
                if args.geo == 'line':
                    _, negative_logit, _, _, idxs = model(None, negative_sample, None, batch_queries_dict, batch_idxs_dict)
                else:
