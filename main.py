...
def parse_args(args=None):
    ...
    parser.add_argument('--geo', default='vec', type=str, choices=['vec', 'box', 'beta', 'line'], help='the reasoning model, vec for GQE, box for Query2box, beta for BetaE')
    ...
    parser.add_argument('-linem', '--line_mode', default="(1600,2)", type=str, help='(hidden_dim,num_layer) for LinE relational projection')
    ...
    parser.add_argument('-evu', '--evaluate_union', default="DNF", type=str, choices=['DNF', 'DM', 'U'], help='the way to evaluate union queries, transform it to disjunctive normal form (DNF) or use the De Morgan\'s laws (DM) or use line union')
    parser.add_argument('-npts', '--number_of_points', default=3, type=int, help='number of points to represent line')
    parser.add_argument('-ql', '--query_loss', default='F', type=str, choices=['F', 'T'], help='whether adding query loss or not')
    parser.add_argument('-att', '--attention', default='F', type=str, choices=['F', 'T'], help='whether adding attention or not')
...
def main(args):
    ...
    if args.evaluate_union == 'DM':
        assert args.geo == 'beta' or args.geo == 'line', "only LinE and BetaE supports modeling union using De Morgan's Laws"
    ...
    if args.geo in ['box']:
        ...
        tmp_str = "g-{}-mode-{}".format(args.gamma, args.line_mode)
    ...
    if args.geo == 'line':
        logging.info('#point: %d' % args.number_of_points)
        logging.info('Query loss and attention: %s, %s' % (args.query_loss, args.attention))
    ...
    model = KGReasoning(
        ...
        query_loss=args.query_loss,
        attention=args.attention,
        box_mode=eval_tuple(args.box_mode),
        beta_mode=eval_tuple(args.beta_mode),
        line_mode=eval_tuple(args.line_mode),
    ...
    if args.geo == 'box':
        ...
        logging.info('line mode = %s' % args.line_mode)
